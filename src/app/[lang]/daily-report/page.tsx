      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
        <header className="flex flex-col gap-4 rounded-2xl border bg-background p-4 shadow-sm md:flex-row md:items-center md:justify-between md:p-5">
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div>
              <h1 className="text-xl font-semibold tracking-tight md:text-2xl">{t.dailyReport.title}</h1>
              <p className="mt-1 text-sm text-muted-foreground">
                {t.dailyReport.description}
              </p>
              <div className="mt-3 flex flex-wrap gap-2 text-xs text-muted-foreground">
                <Badge variant="secondary">{t.dailyReport.markdownOutput}</Badge>
                <Badge variant="outline">{t.dailyReport.tokenStats}</Badge>
              </div>
            </div>
          </div>
        ...
        <Card>
          ...
          <CardContent className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            ...
            <Button type="button" onClick={handleGenerate} disabled={isLoading}>
              ...
              {isLoading ? t.dailyReport.generating : t.dailyReport.generate}
            </Button>
          </CardContent>
        </Card>
        ...
        <Card className="min-h-72">
          <CardHeader>
            <CardTitle className="text-base">{t.dailyReport.contentTitle}</CardTitle>
          </CardHeader>
          <CardContent>
            {isLoading ? (
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">{t.dailyReport.statsTitle}</CardTitle>
          </CardHeader>
          <CardContent>
